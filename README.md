...
- [`rustcpd==4.0.0`](https://pypi.org/project/rustcpd/) for shape-model-guided deformable registration and pose initialization. Shape Completion uses the same exact rustcpd release for single, batch, and calibration workflows.

When a Landmark Transfer workflow first needs these packages, Slicer's `pip_ensure` dependency manager checks their versions and asks before installing or upgrading them with progress feedback. The legacy C++ `tiny3d` distribution and `tiny3d-rs` cannot coexist because they provide the same import package. If the legacy backend is already loaded, Landmark Transfer requests a restart before changing packages; otherwise it removes both conflicting distributions and installs a clean Rust replacement. Both registration modules refuse to replace a different rustcpd version while it is loaded: restart Slicer, allow installation of rustcpd==4.0.0, and restart again when prompted. An internet connection is therefore required the first time these dependencies are installed.
